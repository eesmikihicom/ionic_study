...
// 	}
// 	if ($('#kanzashi_video').length) {
// 		var v = document.querySelector('#kanzashi_video');
// 		v.addEventListener('mouseover', function () {
// 			this.play();
// 		}, false);
// 		v.addEventListener('mouseout', function () {
...
$(function () {
	if ($('video').length) {
		...
		$(".js-modal-btn-tsubaki").modalVideo();
	}
});
$(function () {
	if ($('video').length) {
		...
		$(".js-modal-btn-kanzashi").modalVideo();
	}
});
...
$(window).on("load", function () {
	...
	//$("#topRanking").find(".slick-list").addClass("ranking-slide").removeClass("draggable slick-list");
});
...
------------------------------------------------------*/
$(window).on('load', function () {
	if ($("#sp_productName_top").length) {
		var browserBottom = $(this).height();
		var startPos = 0,
			winScrollTop = 0;
		var prod_name_height = $(".product-name-dokuji3").outerHeight();
		if (window.matchMedia('(max-width: 800px)').matches) {
			$("#fs_form").css({
				"margin-top": prod_name_height
			});
		}

		$(window).on('scroll', function () {
			winScrollTop = $(this).scrollTop()
			var prodNameTop = $("#sp_productName_top").offset().top - winScrollTop + 300;
			if (browserBottom > prodNameTop && winScrollTop >= startPos) {
				$(".product-name-dokuji3").addClass("close");
			} else {
				$(".product-name-dokuji3").removeClass("close");
			}
...
$(window).on('load', function () {
	if ($("#fs_ProductCategory").length || $("#fs_ProductSearch").length) {
		...
		$(window).on('scroll', function () {
			winScrollTop = $(this).scrollTop();
			if (winScrollTop > openTitle && winScrollTop >= startPos) {
				$(".fs-c-heading").addClass("open");
			} else {
				$(".fs-c-heading").removeClass("open");
			}
			startPos = winScrollTop;
		});
...
/* クラシックボトル　電話用
------------------------------------------------------*/
//＜2020.04.10 ネット販売解禁のためコメントアウト＞
// $(window).on('load', function () {
// 	var date = new Date();
// 	var dayOfWeek = date.getDay();
// 	if (document.URL.match('tsubaki003') || document.URL.match('tsubaki004')) {
// 		if (window.matchMedia('(min-width: 579px)').matches) {
// 			$("button.fs-c-button--addToCart--detail").addClass('telOnly');
// 			$('.telOnly .fs-c-button__label').text("電話注文のみ");
// 			$("#original_comment2").insertBefore("#fs_form");

// 		} else {
// 			if (dayOfWeek == 3) {
// 				$("button.fs-c-button--addToCart--detail").wrap('<a class="pushTel" href="tel:0752412525"></a>');
// 			} else {
// 				$("button.fs-c-button--addToCart--detail").wrap('<a class="pushTel" href="tel:0120280323"></a>');
// 			}
// 			$(".pushTel button").remove();
// 			$(".pushTel").append('<div>');
// 			$(".pushTel div").append('<span>');
// 			$(".pushTel div").addClass('fs-c-button--addToCart--detail fs-c-button--primary');
// 			$(".pushTel div span").addClass('fs-c-button__label').text("電話をかける");
// 		}

// 	}
// 	if (document.URL.match('tsubaki003') || document.URL.match('tsubaki004') || document.URL.match('tsubaki014') || document.URL.match('tsubaki016')) {
